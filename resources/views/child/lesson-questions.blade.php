                        ...
                        <div class="event-section">
                            <div class="row">
                                    ...
                                    <!-- Start Event -->
                                        <div class="single__event d-flex" id="question_div_{{$question->id}}">
                                            <div class="event__inner">
                                                @if($question->image)
                                                    <img src="{{asset('')}}uploads/tests/{{$question->image}}"
                                                         alt="" style="width: 50%">
                                                @endif
                                ...
                                <div class="col-md-12">
                                    ...
                                    <button class="btn btn-warning pull-right btn-lg" id="see_results"
                                            style="margin: 0 5px;display: none;">See Answers Results
                                    </button>&nbsp;&nbsp;&nbsp;
                                    <button class="btn btn-success pull-right btn-lg" id="submit_btn"
                                            style="margin: 0 5px;">Submit
                                    </button>&nbsp;&nbsp;&nbsp;
                                </div>
                            </div>
                        </div>
...
@section('extra-js')
            ...
            $('input[name=answer_{{$question->id}}]').click(function () {
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if (res == 1) {
                            score = score + 1;
                            $('#question_div_{{$question->id}}').addClass("success_ans");
                            is_correct = 'correct';
                        } else {
                            $('#question_div_{{$question->id}}').addClass("error_ans");
                            is_correct = 'wrong';
                        }
    ...
    <script>
        $('#submit_btn').click(function () {
            ...
            var que_res = JSON.stringify(all);
            ...
            if (number_of_que > 0) {
                if (que_nos == number_of_que) {
                    $.ajax({
                        url: "/child/test/answer/save",
                        type: 'POST',
                        data: {'score_final': score_final, 'user_id': user_id, 'lesson_id': lesson_id, 'que_res':que_res},
                        success: function (res) {
                            if (res == 'success') {
                                ...
                                $.confirm({
                                    ...
                                    title: 'Success ',
                                    content: 'You completed Test successfully',
                                    theme: 'modern',
                                    closeIcon: true,
                                    animation: 'scale',
                                    type: 'green',
        ...
        $('#see_results').click(function () {
            $('.success_ans').css('background', '#64d772')
            $('.error_ans').css('background', '#d78964')
        });
    </script>
@endsection
