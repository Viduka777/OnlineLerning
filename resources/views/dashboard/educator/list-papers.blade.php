            ...
            <div class="page-title">
                <div class="title_left">
                    <h3>List Lesson Followers <a class="pull-right btn btn-sm btn-info"
                                                 href="{{route('educator.mylessons')}}">Go to All Lessons</a></h3>
                </div>
                        ...
                        <div class="x_content" class="col-md-4 col-sm-12 col-xs-12" style="position: relative;">
                            <table class="table table-striped projects">
                                ...
                                <tr>
                                    ...
                                    <th style="width: 10%">View Paper</th>
                                </tr>
                                </thead>
                                <tbody>
                                @php $x = 1; @endphp
                                @if(count($papers))
                                    @foreach($papers as $key => $paper)
                                        ...
                                        @foreach($paper as $p)
                                            <tr>
                                                @php
                                                    $user = \App\User::where('id',$p->user_id)->first();
                                                    $lesson = \App\Models\Lesson::where('id',$p->lesson_id)->first();
                                                @endphp
                                                <td>{{$x}}</td>
                                                <td>{{$user->name}}</td>
                                                <td>{{$user->email}}</td>
                                                <td>{{$lesson->title}}</td>
                                                <td>{{$p->mark}}</td>
                                                <td><a href="{{asset('')}}educator/lesson/child/view/paper/{{$p->id}}">View</a>
                                                </td>
                                            </tr>
                                        @endforeach
                                        @php $x++; @endphp
